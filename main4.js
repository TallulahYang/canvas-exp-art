...
for (var phi = 0; phi < Math.PI * 2; phi += 0.1) {
    targets.push(new Target(parabola(phi)[0] + canvas.width / 2, heartPosition(phi)[1] + canvas.height / 2, 10));
}
var targetIndex = randint(0, targets.length);
var particle = new Target(targets[targetIndex].x, targets[targetIndex].y, 5, '#ff0000');
